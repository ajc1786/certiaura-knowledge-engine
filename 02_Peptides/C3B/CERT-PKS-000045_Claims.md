<div align="center">

<img src="../Assets/Brand/CertiAura_Brand_Master.png" alt="CertiAura - Evidence. Clarity. Confidence." width="700">

</div>

---

# UNBRANDED CEREBROLYSIN-TYPE PRODUCT FAMILY CLAIM CONTROL MATRIX

| Claim | Permission |
|---|---|
| This asset is in Priority P0 closure. | Allowed |
| Published Cerebrolysin evidence concerns a manufacturer-controlled porcine-brain protein hydrolysate preparation. It does not establish equivalence of an unbranded 60 mg vial or another cerebroprotein hydrolysate. | Allowed as a controlled boundary |
| The catalogue item is equivalent to an authorised product. | False / prohibited |
| Supplier identity, potency and sterility are verified. | False / prohibited |
| A treatment, dosing, injection or reconstitution protocol is validated. | False / prohibited |
| The asset is Platinum Certified. | False / prohibited |

**Status as of:** 2026-07-16
