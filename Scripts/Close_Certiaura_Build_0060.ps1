...
$BuildNumber = "0060"
$Candidate = "RC1"
$Title = "BPC-157 governed evidence reconstruction, regulatory and sport boundary assessment, human-evidence gap control, review-board transition, appeal resolution and repeatable multi-peptide onboarding baseline"
$ExpectedPredecessor = "594152fcfba3b1612b71d7b6e5c23759c906e464"
$ExpectedSubject = "Add Certiaura Build 0060 BPC-157 governed evidence reconstruction, regulatory and sport boundary assessment, human-evidence gap control, review-board transition, appeal resolution and repeatable multi-peptide onboarding baseline"
...
Invoke-CertiauraGitNonInteractiveGuard -Repository $Repository -ScriptBlock {
    ...
    if ($Head -eq $ExpectedPredecessor) {
        ...
        if (@(Compare-Object -ReferenceObject $Owned -DifferenceObject $Staged).Count -ne 0) {
            throw "Staged Build 0060 path set is not exact."
        }
        ...
        Assert-Exit "commit Build 0060"
    }
    elseif ($Subject -ne $ExpectedSubject) {
        throw "Repository is neither the Build 0059 predecessor nor an existing Build 0060 commit."
    }
    ...
    if ($Subject -ne $ExpectedSubject) { throw "Build 0060 subject mismatch." }
    ...
    Write-Host "BUILD_0060_GIT_NONINTERACTIVE_GUARD_VALIDATED"
    Write-Host "NO_MANUAL_GIT_OBJECT_CLEANUP_PROMPTS"
    Write-Host "BUILD_0060_COMMITTED_PUSHED"
    ...
    $Evidence = [ordered]@{
        ...
        endpoint = "BUILD_0060_GITHUB_ACTIONS_GREEN"
        founder_ready_status = "BUILD_0060_READY_FOR_FOUNDER_GREEN"
    }

    New-Item -ItemType Directory -Path $ReportRoot -Force | Out-Null
    $ExternalEvidence = Join-Path $ReportRoot "BUILD_0060_CLOSURE_EVIDENCE.json"
    $LocalEvidenceDirectory = Join-Path $Repository "Documentation\Build_Records\0060\Closure_Evidence"
    New-Item -ItemType Directory -Path $LocalEvidenceDirectory -Force | Out-Null
    $LocalEvidence = Join-Path $LocalEvidenceDirectory "BUILD_0060_CLOSURE_EVIDENCE.json"
    ...
    Write-Host "===== CERTIAURA_BUILD_0060_CLOSURE_EVIDENCE_BEGIN ====="
    ...
    Write-Host "BUILD_0060_GITHUB_ACTIONS_GREEN"
    Write-Host "BUILD_0060_READY_FOR_FOUNDER_GREEN"
    Write-Host "FOUNDER_CONFIRMATION_READY: Reply with exact phrase GREEN"
    Write-Host "===== CERTIAURA_BUILD_0060_CLOSURE_EVIDENCE_END ====="
}
