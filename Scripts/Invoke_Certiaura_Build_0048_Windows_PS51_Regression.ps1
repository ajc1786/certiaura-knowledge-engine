...
$BuildNumber = "0048"
$BuildId = "CERT-BUILD-0048"
$CommitMessage = "Add Certiaura Build 0048 retatrutide clinician review approval, export version control, controlled handoff and audit trail baseline"
$PriorCommitMessage = "Add Certiaura Build 0047 retatrutide controlled longitudinal dashboard, alert review workflow and clinician export baseline"
...
try {
    ...
    if (-not (Test-Path -LiteralPath $Package -PathType Leaf)) {
        throw "Build 0048 package was not found: $Package"
    }
    $ActualPackageSha256 = (
    ...
    if (-not [string]::Equals(
        ...
        throw "Build 0048 package SHA-256 mismatch."
    }
    Write-Host "BUILD 0048 PACKAGE VERIFIED: PASS" -ForegroundColor Green
    Write-Host "Package: $Package"
    Write-Host "SHA-256: $ActualPackageSha256"

    New-Item -ItemType Directory -Path $ReportRoot -Force | Out-Null
    $Stamp = Get-Date -Format "yyyyMMdd_HHmmss"
    $PreflightReport = Join-Path $ReportRoot "BUILD_0048_PREFLIGHT_$Stamp.json"
    $DryRunReport = Join-Path $ReportRoot "BUILD_0048_SYNTHETIC_DRY_RUN_$Stamp.json"
    $InternalBackupReport = Join-Path $ReportRoot "BUILD_0048_INTERNAL_BACKUP_NEGATIVE_$Stamp.json"
    $ApplyReport = Join-Path $ReportRoot "BUILD_0048_SYNTHETIC_APPLY_$Stamp.json"
    $ValidatorReport = Join-Path $ReportRoot "BUILD_0048_SYNTHETIC_VALIDATOR_$Stamp.json"
    $RegressionReport = Join-Path $ReportRoot "BUILD_0048_WINDOWS_PS51_REGRESSION_$Stamp.json"
    $RollbackReport = Join-Path $ReportRoot "BUILD_0048_SYNTHETIC_ROLLBACK_$Stamp.json"
    $SecondDryRunReport = Join-Path $ReportRoot "BUILD_0048_SYNTHETIC_DRY_RUN_AFTER_ROLLBACK_$Stamp.json"
    $SecondApplyReport = Join-Path $ReportRoot "BUILD_0048_SYNTHETIC_APPLY_AFTER_ROLLBACK_$Stamp.json"

    Write-Step "Run CMD parser precheck"
    $TempRoot = Join-Path $env:TEMP (
        "Certiaura_0048_PS51_Regression_" + [guid]::NewGuid().ToString("N")
    )
    New-Item -ItemType Directory -Path $TempRoot -Force | Out-Null
    $CmdPrecheck = Join-Path $TempRoot "BUILD_0048_CMD_PRECHECK.cmd"
    $CmdContent = @"
@echo off
setlocal
if not exist "%~dp0" exit /b 41
echo BUILD 0048 CMD PARSER PRECHECK: PASS
exit /b 0
"@
    ...
    $Preflight = Join-Path $Payload "Scripts\Invoke_Certiaura_Build_0048_Preflight.ps1"
    $Importer = Join-Path $Payload "Scripts\Invoke_Certiaura_Build_0048_Import.ps1"
    $RollbackHelper = Join-Path $Payload "13_Project_Genesis\Import\rollback_build_0048_pending_import.py"
    foreach ($Required in @($Preflight, $Importer, $RollbackHelper)) {
        if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) {
            throw "Build 0048 regression component is missing: $Required"
        }
    }
    ...
    if (-not [bool]$PreflightResult.valid) {
        throw "Build 0048 package preflight report is not valid."
    }
    ...
    $CanonicalRunner = Join-Path $Payload "Scripts\Run_Certiaura_Build_0048.ps1"
    ...
    if ($CanonicalRunnerText -notmatch [regex]::Escape(
        '& $Python -B -m unittest discover -s $TestRoot -p "test_build_0048_retatrutide_clinician_approval_handoff.py"'
    )) {
    ...
    $BuildTestText = Get-Content `
        -LiteralPath (Join-Path $Payload "13_Project_Genesis\Tests\test_build_0048_retatrutide_clinician_approval_handoff.py") `
        -Raw
    if ($BuildTestText -match [regex]::Escape('for p in ROOT.rglob("*")')) {
        throw "Build 0048 hygiene test still scans unrelated historical repository files."
    }
    if ($BuildTestText -notmatch [regex]::Escape('manifest=self.load("Documentation/Build_Records/0048/ASSET_INTENT_MANIFEST.json")')) {
        throw "Build 0048 hygiene test is not scoped through the exact Asset Intent Manifest."
    }
    ...
    $HistoricalPath = Join-Path $SyntheticRepo "Documentation\Historical\HISTORICAL_CONTROL_000047.md"
    $ClosurePath = Join-Path $SyntheticRepo "Documentation\Build_Records\0047\CLOSURE_RECORD.json"
    $Build0047ExportPath = Join-Path $SyntheticRepo "12_Reports\Standards\RETATRUTIDE_CLINICIAN_EXPORT_BASELINE.md"
    $PriorOwnershipHelperPath = Join-Path $SyntheticRepo "13_Project_Genesis\Validators\build_0047_asset_ownership.py"
    ...
    $HistoricalRow = @(
        "CERT-SYS-000047", "Historical unrelated control", "SYS", "HISTORICAL_CONTROL", "ACTIVE", "Certiaura",
        "", "2026-07-20", "Regression fixture", "Documentation/Historical/HISTORICAL_CONTROL_000047.md", "", "1.0.0",
        "100", "", "", "", "", "", "2027-07-20", "Historical fixture", "CERT-BUILD-0047", "0047",
    ...
    Write-Utf8NoBomLf -Path $Build0047ExportPath -Content "# Build 0047 clinician export baseline`n`nBuild provenance: CERT-BUILD-0047`n"
    Write-Utf8NoBomLf -Path $PriorOwnershipHelperPath -Content "BUILD_PROVENANCE = `"CERT-BUILD-0047`"`n"
    ...
    $Build0047ExportHashBefore = (Get-FileHash -LiteralPath $Build0047ExportPath -Algorithm SHA256).Hash
    ...
    Assert-NativeExit "Commit synthetic Build 0047 baseline"
    git -C $SyntheticRepo push -u origin main
    Assert-NativeExit "Push synthetic Build 0047 baseline"

    Write-Step "Resolve exact Build 0047 commit and ancestry"
    $PriorMatches = @(
    ...
    if ($PriorMatches.Count -ne 1) {
        throw "Expected exactly one exact Build 0047 implementation commit; found $($PriorMatches.Count)."
    }
    ...
    Assert-NativeExit "Verify Build 0047 ancestry"
    ...
    if (-not [bool]$DryRun.valid -or $DryRun.transaction_status -ne "DRY_RUN_VALIDATED") {
        throw "Build 0048 synthetic dry-run did not validate."
    }
    if (@($DryRun.register_changes).Count -ne 6) {
        throw "Build 0048 synthetic dry-run did not identify six formal asset changes."
    }
    ...
    if (-not [bool]$ApplyResult.valid -or -not [bool]$ApplyResult.applied) {
        throw "Build 0048 synthetic apply did not validate."
    }
    ...
    Assert-NativeExit "Build 0048 synthetic post-apply rollback"
    ...
    if (-not [bool]$RollbackResult.valid -or $RollbackResult.status -ne "ROLLED_BACK_CLEAN") {
        throw "Build 0048 synthetic rollback report is not clean."
    }
    ...
    if (-not [bool]$SecondDryRunResult.valid -or $SecondDryRunResult.transaction_status -ne "DRY_RUN_VALIDATED") {
        throw "Build 0048 second synthetic dry run did not validate after rollback."
    }
    ...
    if (-not [bool]$SecondApplyResult.valid -or -not [bool]$SecondApplyResult.applied) {
        throw "Build 0048 second synthetic apply did not validate after rollback."
    }
    ...
    $HistoricalRows = @($RegisterRows | Where-Object {
        $_.'Universal Asset Identifier' -ceq "CERT-SYS-000047"
    })
    if ($HistoricalRows.Count -ne 1) {
        throw "Historical UAI 000047 fixture was not preserved exactly once."
    }
    if ($HistoricalRows[0].'Build Provenance' -ne "CERT-BUILD-0047") {
        throw "Historical UAI 000047 fixture was not preserved as Build 0047 provenance."
    }
    $BuildRows = @($RegisterRows | Where-Object {
        $_.'Build Provenance' -ceq $BuildId
    })
    if ($BuildRows.Count -ne 6) {
        throw "Expected six exact Build 0048 register rows; found $($BuildRows.Count)."
    }

    Write-Step "Run exact ownership validator and automated tests"
    $env:PYTHONDONTWRITEBYTECODE = "1"
    $Build0047ExportHashAfter = (Get-FileHash -LiteralPath $Build0047ExportPath -Algorithm SHA256).Hash
    ...
    if ($UnrelatedLegacyCrlfHashAfter -ne $UnrelatedLegacyCrlfHashBefore) {
        throw "Build 0048 modified the unrelated historical CRLF fixture."
    }
    if ($Build0047ExportHashAfter -ne $Build0047ExportHashBefore) {
        throw "Build 0047 clinician export baseline was modified by Build 0048."
    }
    if ($PriorOwnershipHashAfter -ne $PriorOwnershipHashBefore) {
        throw "Build 0047 ownership helper was modified by Build 0048."
    }

    $Validator = Join-Path $SyntheticRepo "13_Project_Genesis\Validators\validate_retatrutide_clinician_approval_handoff.py"
    $TestModule = Join-Path $SyntheticRepo "13_Project_Genesis\Tests\test_build_0048_retatrutide_clinician_approval_handoff.py"
    & $Python -B $Validator $SyntheticRepo --report $ValidatorReport
    Assert-NativeExit "Build 0048 exact ownership validator"
    & $Python -B $TestModule
    Assert-NativeExit "Build 0048 automated tests"
    ...
    git -C $SyntheticRepo add -A
    Assert-NativeExit "Stage synthetic Build 0048"
    ...
    Write-Step "Commit and push synthetic Build 0048"
    git -C $SyntheticRepo commit -m $CommitMessage
    Assert-NativeExit "Commit synthetic Build 0048"
    git -C $SyntheticRepo push origin main
    Assert-NativeExit "Push synthetic Build 0048"
    $RemoteSubject = (
    ...
    if ($RemoteSubject -cne $CommitMessage) {
        throw "Synthetic remote commit subject does not match the locked Build 0048 message."
    }
    ...
    $Result = [ordered]@{
        ...
        historical_uai_000047 = "PRESERVED_AND_EXCLUDED"
        build_0047_clinician_export_baseline = "PRESERVED_UNCHANGED"
        build_0047_ownership_helper = "PRESERVED_UNCHANGED"
        dry_run_collision_inventory = "PASS"
        exact_provenance_validator = "PASS"
        automated_tests = "16_OF_16_PASS"
    ...
    Write-Host ""
    Write-Host "BUILD 0048 WINDOWS POWERSHELL 5.1 REGRESSION: PASS" -ForegroundColor Green
    Write-Host "Regression report: $RegressionReport"
    Write-Host "Canonical repository modified: NO"
}
catch {
    Write-Host ""
    Write-Host "BUILD 0048 WINDOWS POWERSHELL 5.1 REGRESSION: FAIL" -ForegroundColor Red
