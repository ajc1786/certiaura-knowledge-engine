...
$TempRoot = Join-Path $env:TEMP ("Certiaura_0049_Preflight_" + [guid]::NewGuid().ToString("N"))
try {
    Expand-Archive -LiteralPath $Package -DestinationPath $TempRoot -Force
    $Python = Resolve-Python
    $PackageVerifier = Join-Path $TempRoot "13_Project_Genesis\Validators\verify_build_0049_package.py"
    $Validator = Join-Path $TempRoot "13_Project_Genesis\Validators\validate_retatrutide_follow_up_feedback_audit.py"
    foreach ($Required in @($PackageVerifier, $Validator)) { if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) { throw "Build 0049 validation component is missing: $Required" } }
    $env:PYTHONDONTWRITEBYTECODE = "1"
    & $Python -B $PackageVerifier $Package
    if ($LASTEXITCODE -ne 0) { throw "Build 0049 package checksum and hygiene verification failed." }
    & $Python -B $Validator $TempRoot --report $Report
    if ($LASTEXITCODE -ne 0) { throw "Build 0049 package validation failed." }
    ...
    Write-Host "Build 0049 preflight passed." -ForegroundColor Green
}
finally {
