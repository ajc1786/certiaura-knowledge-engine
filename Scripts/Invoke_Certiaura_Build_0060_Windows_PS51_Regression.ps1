...
$TempRoot = Join-Path `
    $env:TEMP `
    ("CERTIAURA_0060_PS51_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $Importer = Join-Path `
        $Extract `
        "Scripts\Invoke_Certiaura_Build_0060_Import.ps1"

    $ForcedReport = Join-Path `
        $ReportRoot `
        ("BUILD_0060_FORCED_ROLLBACK_" + $Timestamp + ".json")
    ...
    $CleanReport = Join-Path `
        $ReportRoot `
        ("BUILD_0060_CLEAN_REAPPLY_" + $Timestamp + ".json")
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone Build 0060 repository validation failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone full Build 0060 test suite failed before canonical apply."
    }

    Write-Host "BUILD_0060_PRE_CANONICAL_FULL_SUITE_VALIDATED" -ForegroundColor Green

    $ManifestPath = Join-Path `
        $Clone `
        "Documentation\Build_Records\0060\ASSET_INTENT_MANIFEST.json"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Temporary clone contains changes outside Build 0060 ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot ("BUILD_0060_PS51_BYTE_EQUALITY_" + $Timestamp + ".json"))
    )
    ...
    Write-Host "BUILD_0060_GIT_NONINTERACTIVE_GUARD_VALIDATED" -ForegroundColor Green
    ...
    & git -C $Clone remote add build0060_regression $BareRemote
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary remote configuration failed."
    }
    Invoke-CertiauraGitNonInteractiveGuard -Repository $Clone -ScriptBlock {
        & git -C $Clone push build0060_regression HEAD:refs/heads/build0060-regression
        if ($LASTEXITCODE -ne 0) { throw "Temporary push-path regression failed." }
    }

    Write-Host "BUILD_0060_WINDOWS_PS51_REGRESSION_PASS" -ForegroundColor Green
}
finally {
