...
try {
    ...
    if (-not (Test-Path -LiteralPath $Package -PathType Leaf)) {
        throw "Build 0047 package was not found: $Package"
    }
    $ActualPackageSha256 = (
    ...
    if (-not [string]::Equals(
        ...
        throw "Build 0047 package SHA-256 mismatch."
    }
    Write-Host "BUILD 0047 PACKAGE VERIFIED: PASS" -ForegroundColor Green
    ...
    $DryRunReport = Join-Path $ReportRoot "BUILD_0047_SYNTHETIC_DRY_RUN_$Stamp.json"
    $InternalBackupReport = Join-Path $ReportRoot "BUILD_0047_INTERNAL_BACKUP_NEGATIVE_$Stamp.json"
    $ApplyReport = Join-Path $ReportRoot "BUILD_0047_SYNTHETIC_APPLY_$Stamp.json"
    $ValidatorReport = Join-Path $ReportRoot "BUILD_0047_SYNTHETIC_VALIDATOR_$Stamp.json"
    $RegressionReport = Join-Path $ReportRoot "BUILD_0047_WINDOWS_PS51_REGRESSION_$Stamp.json"
    $RollbackReport = Join-Path $ReportRoot "BUILD_0047_SYNTHETIC_ROLLBACK_$Stamp.json"
    $SecondDryRunReport = Join-Path $ReportRoot "BUILD_0047_SYNTHETIC_DRY_RUN_AFTER_ROLLBACK_$Stamp.json"
    $SecondApplyReport = Join-Path $ReportRoot "BUILD_0047_SYNTHETIC_APPLY_AFTER_ROLLBACK_$Stamp.json"

    Write-Step "Run CMD parser precheck"
    $TempRoot = Join-Path $env:TEMP (
        "Certiaura_0047_PS51_Regression_" + [guid]::NewGuid().ToString("N")
    )
    New-Item -ItemType Directory -Path $TempRoot -Force | Out-Null
    $CmdPrecheck = Join-Path $TempRoot "BUILD_0047_CMD_PRECHECK.cmd"
    $CmdContent = @"
@echo off
setlocal
if not exist "%~dp0" exit /b 41
echo BUILD 0047 CMD PARSER PRECHECK: PASS
exit /b 0
"@
    ...
    $Preflight = Join-Path $Payload "Scripts\Invoke_Certiaura_Build_0047_Preflight.ps1"
    $Importer = Join-Path $Payload "Scripts\Invoke_Certiaura_Build_0047_Import.ps1"
    $RollbackHelper = Join-Path $Payload "13_Project_Genesis\Import\rollback_build_0047_pending_import.py"
    foreach ($Required in @($Preflight, $Importer, $RollbackHelper)) {
        if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) {
            throw "Build 0047 regression component is missing: $Required"
        }
    }
    ...
    if (-not [bool]$PreflightResult.valid) {
        throw "Build 0047 package preflight report is not valid."
    }
    ...
    $CanonicalRunner = Join-Path $Payload "Scripts\Run_Certiaura_Build_0047.ps1"
    ...
    if ($CanonicalRunnerText -notmatch [regex]::Escape(
        ...
        throw "Canonical launcher does not contain the approved PS5.1-safe list conversion."
    }
    if ($CanonicalRunnerText -match [regex]::Escape(
        '$ApprovedMatches = @($Matches)'
    )) {
        throw "Canonical launcher still contains the incompatible generic-list unary-array conversion."
    }
    if ($CanonicalRunnerText -notmatch [regex]::Escape(
    ...
    $HistoricalPath = Join-Path $SyntheticRepo "Documentation\Historical\HISTORICAL_CONTROL_000046.md"
    $ClosurePath = Join-Path $SyntheticRepo "Documentation\Build_Records\0046\CLOSURE_RECORD.json"
    $Build0045SchemaPath = Join-Path $SyntheticRepo "Schemas\retatrutide_longitudinal_journey.schema.json"
    $LegacyOwnershipHelperPath = Join-Path $SyntheticRepo "13_Project_Genesis\Validators\build_asset_ownership.py"
    ...
    $HistoricalRow = @(
        "CERT-SYS-000046", "Historical unrelated control", "SYS", "HISTORICAL_CONTROL", "ACTIVE", "Certiaura",
        "", "2026-07-20", "Regression fixture", "Documentation/Historical/HISTORICAL_CONTROL_000046.md", "", "1.0.0",
        "100", "", "", "", "", "", "2027-07-20", "Historical fixture", "CERT-BUILD-0046", "0046",
    ...
    Write-Utf8NoBomLf -Path $Build0045SchemaPath -Content "{`n  `"build_provenance`": `"CERT-BUILD-0045`"`n}`n"
    Write-Utf8NoBomLf -Path $LegacyOwnershipHelperPath -Content "BUILD_PROVENANCE = `"CERT-BUILD-0046`"`n"
    $Build0045SchemaHashBefore = (Get-FileHash -LiteralPath $Build0045SchemaPath -Algorithm SHA256).Hash
    $LegacyOwnershipHashBefore = (Get-FileHash -LiteralPath $LegacyOwnershipHelperPath -Algorithm SHA256).Hash
    $Closure = [ordered]@{
        build = "0046"
    ...
    Assert-NativeExit "Commit synthetic Build 0046 baseline"
    git -C $SyntheticRepo push -u origin main
    Assert-NativeExit "Push synthetic Build 0046 baseline"

    Write-Step "Resolve exact Build 0046 commit and ancestry"
    $PriorMatches = @(
    ...
    if ($PriorMatches.Count -ne 1) {
        throw "Expected exactly one exact Build 0046 implementation commit; found $($PriorMatches.Count)."
    }
    ...
    Assert-NativeExit "Verify Build 0046 ancestry"
    ...
    if (-not [bool]$DryRun.valid -or $DryRun.transaction_status -ne "DRY_RUN_VALIDATED") {
        throw "Build 0047 synthetic dry-run did not validate."
    }
    if (@($DryRun.register_changes).Count -ne 6) {
        throw "Build 0047 synthetic dry-run did not identify six formal asset changes."
    }
    ...
    if (-not [bool]$ApplyResult.valid -or -not [bool]$ApplyResult.applied) {
        throw "Build 0047 synthetic apply did not validate."
    }
    ...
    Assert-NativeExit "Build 0047 synthetic post-apply rollback"
    ...
    if (-not [bool]$RollbackResult.valid -or $RollbackResult.status -ne "ROLLED_BACK_CLEAN") {
        throw "Build 0047 synthetic rollback report is not clean."
    }
    ...
    if (-not [bool]$SecondDryRunResult.valid -or $SecondDryRunResult.transaction_status -ne "DRY_RUN_VALIDATED") {
        throw "Build 0047 second synthetic dry run did not validate after rollback."
    }
    ...
    if (-not [bool]$SecondApplyResult.valid -or -not [bool]$SecondApplyResult.applied) {
        throw "Build 0047 second synthetic apply did not validate after rollback."
    }
    ...
    $HistoricalRows = @($RegisterRows | Where-Object {
        $_.'Universal Asset Identifier' -ceq "CERT-SYS-000046"
    })
    if ($HistoricalRows.Count -ne 1) {
        throw "Historical UAI 000046 fixture was not preserved exactly once."
    }
    if ($HistoricalRows[0].'Build Provenance' -ne "CERT-BUILD-0046") {
        throw "Historical UAI 000046 fixture was incorrectly claimed by Build 0047."
    }
    $BuildRows = @($RegisterRows | Where-Object {
        $_.'Build Provenance' -ceq $BuildId
    })
    if ($BuildRows.Count -ne 6) {
        throw "Expected six exact Build 0047 register rows; found $($BuildRows.Count)."
    }

    Write-Step "Run exact ownership validator and automated tests"
    $env:PYTHONDONTWRITEBYTECODE = "1"
    $Build0045SchemaHashAfter = (Get-FileHash -LiteralPath $Build0045SchemaPath -Algorithm SHA256).Hash
    $LegacyOwnershipHashAfter = (Get-FileHash -LiteralPath $LegacyOwnershipHelperPath -Algorithm SHA256).Hash
    if ($Build0045SchemaHashAfter -ne $Build0045SchemaHashBefore) {
        throw "Build 0045 longitudinal journey schema was modified by Build 0047."
    }
    if ($LegacyOwnershipHashAfter -ne $LegacyOwnershipHashBefore) {
        throw "Pre-existing shared ownership helper was modified by Build 0047."
    }

    $Validator = Join-Path $SyntheticRepo "13_Project_Genesis\Validators\validate_retatrutide_longitudinal_dashboard.py"
    $TestModule = Join-Path $SyntheticRepo "13_Project_Genesis\Tests\test_build_0047_retatrutide_dashboard.py"
    & $Python -B $Validator $SyntheticRepo --report $ValidatorReport
    Assert-NativeExit "Build 0047 exact ownership validator"
    & $Python -B $TestModule
    Assert-NativeExit "Build 0047 automated tests"
    ...
    git -C $SyntheticRepo add -A
    Assert-NativeExit "Stage synthetic Build 0047"
    ...
    Write-Step "Commit and push synthetic Build 0047"
    git -C $SyntheticRepo commit -m $CommitMessage
    Assert-NativeExit "Commit synthetic Build 0047"
    git -C $SyntheticRepo push origin main
    Assert-NativeExit "Push synthetic Build 0047"
    $RemoteSubject = (
    ...
    if ($RemoteSubject -cne $CommitMessage) {
        throw "Synthetic remote commit subject does not match the locked Build 0047 message."
    }
    ...
    $Result = [ordered]@{
        ...
        historical_uai_000046 = "PRESERVED_AND_EXCLUDED"
        build_0045_longitudinal_schema = "PRESERVED_UNCHANGED"
        preexisting_ownership_helper = "PRESERVED_UNCHANGED"
        dry_run_collision_inventory = "PASS"
        exact_provenance_validator = "PASS"
        automated_tests = "11_OF_11_PASS"
    ...
    Write-Host ""
    Write-Host "BUILD 0047 WINDOWS POWERSHELL 5.1 REGRESSION: PASS" -ForegroundColor Green
    Write-Host "Regression report: $RegressionReport"
    Write-Host "Canonical repository modified: NO"
}
catch {
    Write-Host ""
    Write-Host "BUILD 0047 WINDOWS POWERSHELL 5.1 REGRESSION: FAIL" -ForegroundColor Red
