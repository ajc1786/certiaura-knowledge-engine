...
$ExpectedHead = "977829a987baf744beab4762478d9f0a88165fb0"
$ExpectedCommit = "Add Certiaura Build 0056 tesamorelin evidence corpus mapping, biological and safety boundary definition, target-specific data contracts, monitoring model and controlled pilot acceptance baseline"
...
function Assert-OptionalManifestPropertyHandling {
    ...
    Write-Host "BUILD_0056_OPTIONAL_PROPERTY_STRICTMODE_VALIDATED" -ForegroundColor Green
}
...
if ($Hash -ne $ExpectedPackageSha256) {
    throw "Build 0056 package SHA-256 mismatch. Expected $ExpectedPackageSha256; observed $Hash."
}

$InitialStatus = @(git -C $Repository status --porcelain)
Assert-Exit "read initial repository status"
if ($InitialStatus.Count -gt 0) {
    throw "Repository must be clean before Build 0056."
}
...
if ($Head -ne $ExpectedHead) {
    throw "Repository HEAD $Head does not equal closed Build 0055 commit $ExpectedHead."
}

$Temp = Join-Path $env:TEMP ("CERTIAURA_0056_" + [guid]::NewGuid().ToString("N"))
...
$CanonicalImportReport = Join-Path $ReportRoot "BUILD_0056_CANONICAL_IMPORT.json"
$AutomaticRollbackReport = Join-Path $ReportRoot "BUILD_0056_AUTOMATIC_ROLLBACK.json"

try {
    New-Item -ItemType Directory -Path $Extract -Force | Out-Null
    Expand-Archive -LiteralPath $PackagePath -DestinationPath $Extract -Force

    $Preflight = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0056_Preflight.ps1"
    $Regression = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0056_Windows_PS51_Regression.ps1"
    $Importer = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0056_Import.ps1"

    & $Preflight `
        -PackagePath $PackagePath `
        -ReportPath (Join-Path $ReportRoot "BUILD_0056_PREFLIGHT.json")

    & $Importer `
        -PackagePath $PackagePath `
        -Repository $Repository `
        -BackupRoot $BackupRoot `
        -ReportPath (Join-Path $ReportRoot "BUILD_0056_CANONICAL_DRY_RUN.json")
    ...
    $Validator = Join-Path $Repository "13_Project_Genesis\Validators\validate_build_0056_repository.py"
    $ManifestPath = Join-Path $Repository "Documentation\Build_Records\0056\ASSET_INTENT_MANIFEST.json"
    $RepositoryReport = Join-Path $Repository "Documentation\Build_Records\0056\POST_IMPORT_REPOSITORY_VALIDATION.json"
    ...
    Assert-Exit "Build 0056 repository validation"
    ...
    Assert-Exit "stage Build 0056 owned paths"
    ...
    Assert-Exit "read staged Build 0056 paths"
    ...
    Assert-Exit "read actual Build 0056 change-set"
    ...
    Assert-Exit "read untracked paths after Build 0056 staging"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Changes exist outside Build 0056 Asset Intent Manifest ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    if ($UnexpectedNoOp.Count -ne 0) {
        throw (
            "Unexpected no-op Build 0056 owned paths: " +
            ($UnexpectedNoOp -join ", ")
        )
    }
    ...
    if ($Difference.Count -ne 0) {
        throw "Final staged change-set does not exactly equal the actual Build 0056 Git change-set."
    }

    Write-Host (
        "BUILD_0056_STAGED_CHANGESET_VALIDATED owned=" +
        $Owned.Count +
        " staged=" +
        $Observed.Count +
        " approved_no_op=" +
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot "BUILD_0056_STAGED_BYTE_EQUALITY.json")
    )
    & $Python.Path @ByteArguments
    Assert-Exit "raw staged byte equality"

    $CommitFile = Join-Path $Repository "Documentation\Build_Records\0056\COMMIT_MESSAGE.txt"
    if ((Get-Content -LiteralPath $CommitFile -Raw).Trim() -ne $ExpectedCommit) {
        throw "Reserved commit subject mismatch."
    }

    Write-Host ""
    Write-Host "BUILD_0056_CANONICAL_VALIDATED_AND_STAGED" -ForegroundColor Green
    ...
    if ($CanonicalApplied) {
        try {
            ...
            Write-Host "BUILD_0056_AUTOMATIC_ROLLBACK_STATE_EXACT" -ForegroundColor Yellow
        }
        catch {
            throw (
                "Build 0056 post-import gate failed and automatic rollback also failed. " +
