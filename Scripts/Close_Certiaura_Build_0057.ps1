...
$ExpectedPredecessor="2e6fe434bf1d0566bf3d1afa33bae24ce13b3b44"; $ExpectedSubject="Add Certiaura Build 0057 tesamorelin governed evidence ingestion, provenance validation, monitoring workflow simulation, safety escalation, audit replay and controlled acceptance baseline"; $WorkflowName="Certiaura Repository Validation"
...
Invoke-CertiauraGitNonInteractiveGuard -Repository $Repository -ScriptBlock {
 ...
 if($Head -eq $ExpectedPredecessor){
  ...
  $ManifestPath=Join-Path $Repository "Documentation\Build_Records\0057\ASSET_INTENT_MANIFEST.json"; $Manifest=Get-Content -LiteralPath $ManifestPath -Raw | ConvertFrom-Json
  ...
  if(@(Compare-Object -ReferenceObject $Owned -DifferenceObject $Staged).Count -ne 0){throw "Staged Build 0057 path set is not exact."}
  ...
  & git -C $Repository commit -m $ExpectedSubject; Assert-Exit "commit Build 0057"
 } elseif($Subject -ne $ExpectedSubject){throw "Repository is neither the Build 0056 predecessor nor an existing Build 0057 commit."}
 $Head=(& git -C $Repository rev-parse HEAD).Trim(); $Subject=(& git -C $Repository log -1 --format=%s).Trim(); if($Subject -ne $ExpectedSubject){throw "Build 0057 subject mismatch."}
 ...
 Write-Host "BUILD_0057_COMMITTED_PUSHED" -ForegroundColor Green; Write-Host "Canonical commit:"; Write-Host $Head
 ...
 New-Item -ItemType Directory -Path $ReportRoot -Force|Out-Null; $Evidence=[ordered]@{build_number="0057";canonical_commit=$Head;commit_subject=$Subject;run_id=[string]$Run.id;workflow_name=[string]$Run.name;status=[string]$Run.status;conclusion=[string]$Run.conclusion;run_attempt=[int]$Run.run_attempt;branch=[string]$Run.head_branch;event=[string]$Run.event;created_at=[string]$Run.created_at;updated_at=[string]$Run.updated_at;repository_clean=$true;git_noninteractive_guard=$true;founder_phrase_required="GREEN"}
 $Evidence|ConvertTo-Json -Depth 8|Set-Content -LiteralPath (Join-Path $ReportRoot "BUILD_0057_GITHUB_ACTIONS_GREEN.json") -Encoding UTF8
 Write-Host "BUILD_0057_GITHUB_ACTIONS_GREEN" -ForegroundColor Green; Write-Host "Run ID:"; Write-Host $Run.id; Write-Host "Canonical commit:"; Write-Host $Head
}
