...
$TempRoot = Join-Path `
    $env:TEMP `
    ("CERTIAURA_0058_PS51_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $Importer = Join-Path `
        $Extract `
        "Scripts\Invoke_Certiaura_Build_0058_Import.ps1"

    $ForcedReport = Join-Path `
        $ReportRoot `
        ("BUILD_0058_FORCED_ROLLBACK_" + $Timestamp + ".json")
    ...
    $CleanReport = Join-Path `
        $ReportRoot `
        ("BUILD_0058_CLEAN_REAPPLY_" + $Timestamp + ".json")
    ...
    $RepositoryValidator = Join-Path `
        $Clone `
        "13_Project_Genesis\Validators\validate_build_0058_repository.py"
    $RepositoryReport = Join-Path `
        $Clone `
        "Documentation\Build_Records\0058\POST_IMPORT_REPOSITORY_VALIDATION.json"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone Build 0058 repository validation failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone full Build 0058 test suite failed before canonical apply."
    }

    Write-Host "BUILD_0058_PRE_CANONICAL_FULL_SUITE_VALIDATED" -ForegroundColor Green

    $ManifestPath = Join-Path `
        $Clone `
        "Documentation\Build_Records\0058\ASSET_INTENT_MANIFEST.json"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Temporary clone contains changes outside Build 0058 ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot ("BUILD_0058_PS51_BYTE_EQUALITY_" + $Timestamp + ".json"))
    )
    ...
    & git -C $Clone remote add build0058_regression $BareRemote
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary remote configuration failed."
    }
    Invoke-CertiauraGitNonInteractiveGuard -Repository $Clone -ScriptBlock {
        & git -C $Clone push build0058_regression HEAD:refs/heads/build0058-regression
        if ($LASTEXITCODE -ne 0) { throw "Temporary push-path regression failed." }
    }

    Write-Host "BUILD_0058_WINDOWS_PS51_REGRESSION_PASS" -ForegroundColor Green
}
finally {
