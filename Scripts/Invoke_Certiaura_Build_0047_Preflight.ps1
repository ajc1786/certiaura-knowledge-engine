...
$TempRoot = Join-Path $env:TEMP ("Certiaura_0047_Preflight_" + [guid]::NewGuid().ToString("N"))
try {
    Expand-Archive -LiteralPath $Package -DestinationPath $TempRoot -Force
    $Python = Resolve-Python
    $PackageVerifier = Join-Path $TempRoot "13_Project_Genesis\Validators\verify_build_0047_package.py"
    $Validator = Join-Path $TempRoot "13_Project_Genesis\Validators\validate_retatrutide_longitudinal_dashboard.py"
    foreach ($Required in @($PackageVerifier, $Validator)) { if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) { throw "Build 0047 validation component is missing: $Required" } }
    $env:PYTHONDONTWRITEBYTECODE = "1"
    & $Python -B $PackageVerifier $Package
    if ($LASTEXITCODE -ne 0) { throw "Build 0047 package checksum and hygiene verification failed." }
    & $Python -B $Validator $TempRoot --report $Report
    if ($LASTEXITCODE -ne 0) { throw "Build 0047 package validation failed." }
    ...
    Write-Host "Build 0047 preflight passed." -ForegroundColor Green
}
finally {
