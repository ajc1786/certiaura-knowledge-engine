...
param(
    ...
    [string]$ReportRoot = "$env:USERPROFILE\OneDrive\Documents\CERTIAURA\Build_Reports\0055"
)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

$ExpectedHead = "8a9dba6615448c1ad85c44cdf89269a8fc3e00e8"
$ExpectedCommit = "Add Certiaura Build 0055 historical GitHub Actions evidence reconciliation, retatrutide baseline closure, reusable architecture handoff and governed next-peptide transition baseline"
...
function Assert-OptionalManifestPropertyHandling {
    ...
    Write-Host "BUILD_0055_OPTIONAL_PROPERTY_STRICTMODE_VALIDATED" -ForegroundColor Green
}
...
if ($Hash -ne $ExpectedPackageSha256) {
    throw "Build 0055 package SHA-256 mismatch. Expected $ExpectedPackageSha256; observed $Hash."
}

$InitialStatus = @(git -C $Repository status --porcelain)
Assert-Exit "read initial repository status"
if ($InitialStatus.Count -gt 0) {
    throw "Repository must be clean before Build 0055."
}
...
if ($Head -ne $ExpectedHead) {
    throw "Repository HEAD $Head does not equal closed Build 0054 commit $ExpectedHead."
}

$Temp = Join-Path $env:TEMP ("CERTIAURA_0055_" + [guid]::NewGuid().ToString("N"))
...
$CanonicalImportReport = Join-Path $ReportRoot "BUILD_0055_CANONICAL_IMPORT.json"
$AutomaticRollbackReport = Join-Path $ReportRoot "BUILD_0055_AUTOMATIC_ROLLBACK.json"
$HistoricalAuditDirectory = Join-Path $ReportRoot "HISTORICAL_ACTIONS_AUDIT"

try {
    New-Item -ItemType Directory -Path $Extract -Force | Out-Null
    Expand-Archive -LiteralPath $PackagePath -DestinationPath $Extract -Force

    $Preflight = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0055_Preflight.ps1"
    $Regression = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0055_Windows_PS51_Regression.ps1"
    $Importer = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0055_Import.ps1"
    $HistoricalAudit = Join-Path $Extract "Scripts\Invoke_Certiaura_Historical_Actions_Audit.ps1"

    & $HistoricalAudit `
        -Repository $Repository `
        -OutputDirectory $HistoricalAuditDirectory

    & $Preflight `
        -PackagePath $PackagePath `
        -ReportPath (Join-Path $ReportRoot "BUILD_0055_PREFLIGHT.json")

    & $Importer `
        -PackagePath $PackagePath `
        -Repository $Repository `
        -BackupRoot $BackupRoot `
        -ReportPath (Join-Path $ReportRoot "BUILD_0055_CANONICAL_DRY_RUN.json") `
        -HistoricalAuditDirectory $HistoricalAuditDirectory
    ...
    $Validator = Join-Path $Repository "13_Project_Genesis\Validators\validate_build_0055_repository.py"
    $ManifestPath = Join-Path $Repository "Documentation\Build_Records\0055\ASSET_INTENT_MANIFEST.json"
    $RepositoryReport = Join-Path $Repository "Documentation\Build_Records\0055\POST_IMPORT_REPOSITORY_VALIDATION.json"
    ...
    Assert-Exit "Build 0055 repository validation"
    ...
    Assert-Exit "stage Build 0055 owned paths"
    ...
    Assert-Exit "read staged Build 0055 paths"
    ...
    Assert-Exit "read actual Build 0055 change-set"
    ...
    Assert-Exit "read untracked paths after Build 0055 staging"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Changes exist outside Build 0055 Asset Intent Manifest ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    if ($UnexpectedNoOp.Count -ne 0) {
        throw (
            "Unexpected no-op Build 0055 owned paths: " +
            ($UnexpectedNoOp -join ", ")
        )
    }
    ...
    if ($Difference.Count -ne 0) {
        throw "Final staged change-set does not exactly equal the actual Build 0055 Git change-set."
    }

    Write-Host (
        "BUILD_0055_STAGED_CHANGESET_VALIDATED owned=" +
        $Owned.Count +
        " staged=" +
        $Observed.Count +
        " approved_no_op=" +
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot "BUILD_0055_STAGED_BYTE_EQUALITY.json")
    )
    & $Python.Path @ByteArguments
    Assert-Exit "raw staged byte equality"

    $CommitFile = Join-Path $Repository "Documentation\Build_Records\0055\COMMIT_MESSAGE.txt"
    if ((Get-Content -LiteralPath $CommitFile -Raw).Trim() -ne $ExpectedCommit) {
        throw "Reserved commit subject mismatch."
    }

    Write-Host ""
    Write-Host "BUILD_0055_CANONICAL_VALIDATED_AND_STAGED" -ForegroundColor Green
    ...
    if ($CanonicalApplied) {
        try {
            ...
            Write-Host "BUILD_0055_AUTOMATIC_ROLLBACK_STATE_EXACT" -ForegroundColor Yellow
        }
        catch {
            throw (
                "Build 0055 post-import gate failed and automatic rollback also failed. " +
