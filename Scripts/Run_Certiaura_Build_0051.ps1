...
param(
    ...
    [string]$ReportRoot = "$env:USERPROFILE\OneDrive\Documents\CERTIAURA\Build_Reports\0051"
)
...
$PackageNameHint = "Certiaura_Build_0051_Retatrutide_Post_Closure_Surveillance.zip"
$ExpectedAssetIntentManifestSha256 = "37165A4AA5DF7004CF23E6A3274D6A7C38059B87162B75FF417637D96D07C799"
...
function Resolve-Package([string]$ExpectedManifestSha256, [string]$RegressionReportRoot) {
    ...
    foreach ($Root in $Roots) {
        $Candidates = @(
            Get-ChildItem `
                -LiteralPath $Root `
                -Filter "Certiaura_Build_0051_Retatrutide_Post_Closure_Surveillance*.zip" `
                -File `
    ...
    if ($MatchesArray.Count -eq 0) {
        throw "No Build 0051 ZIP matches Asset Intent Manifest SHA-256 $ExpectedManifestSha256. Place $PackageNameHint in Downloads and rerun."
    }
    ...
    if ($DistinctPackageHashes.Count -gt 1) {
        ...
        $RegressionReports = @(
            Get-ChildItem `
                ...
                -File `
                -Filter "BUILD_0051_WINDOWS_PS51_REGRESSION_*.json" `
                -ErrorAction SilentlyContinue |
            Sort-Object LastWriteTime -Descending
        )
        if ($RegressionReports.Count -eq 0) {
            throw "Multiple distinct Build 0051 packages were found and no Windows PowerShell 5.1 PASS report identifies the approved package."
        }
        ...
        if ($LatestRegression.status -ne "PASS") {
            throw "Multiple distinct Build 0051 packages were found and the latest Windows PowerShell 5.1 regression report is not PASS."
        }
        $ApprovedPackageHash = (
        ...
        if ($ApprovedMatches.Count -eq 0) {
            throw "The latest passed Build 0051 regression report does not match any available Build 0051 package."
        }
        Write-Warning (
            "Multiple Build 0051 package revisions were found. " +
    ...
    if ($ApprovedMatches.Count -gt 1) {
        Write-Warning (
            "Found $($ApprovedMatches.Count) byte-identical copies of the approved Build 0051 package. " +
...
try {
    ...
    Write-Step "Verify clean canonical repository and Build 0051 closure"
    ...
    $ClosureCandidates = @(@(
        (Join-Path $Repository "Documentation\Build_Records\0050\CLOSURE_RECORD.json"),
        (Join-Path $Repository "Documentation\Build_Records\0050\PACKAGE_VERSION.json"),
        (Join-Path $Repository "Documentation\Build_Records\0050\CONTINUITY_CHECKPOINT_DELTA.json"),
        (Join-Path $Repository "Documentation\Build_Records\0050\PRODUCTION_DASHBOARD_UPDATE.json"),
        (Join-Path $Repository "Documentation\Build_Records\0050\LESSONS_LEARNED_REVIEW.md"),
        (Join-Path $Repository "Documentation\Build_Records\0050\BUILD_MANIFEST.json"),
        (Join-Path $Repository "00_Governance\CERTIAURA_LOCKED_BUILD_CONTINUITY_AND_CHECKPOINT.md")
    ) | Where-Object { Test-Path -LiteralPath $_ -PathType Leaf })
    if ($ClosureCandidates.Count -eq 0) { throw "No Build 0051 checkpoint record was found." }
    ...
    if ($ClosureText -notmatch "ACTIONS_GREEN_CLOSED") {
        Write-Warning "No local Build 0051 ACTIONS_GREEN_CLOSED marker was found. Exact Build 0051 commit and ancestry verification remain mandatory under the founder-confirmed closure carried into Build 0051."
    }
    ...
    Assert-Exit "Resolve exact Build 0051 implementation commit"
    if ($PriorCommitMatches.Count -ne 1) {
        throw "Expected exactly one exact Build 0051 implementation commit; found $($PriorCommitMatches.Count)."
    }
    $PriorCommit = $PriorCommitMatches[0]
    git -C $Repository merge-base --is-ancestor $PriorCommit $ExpectedHead
    Assert-Exit "Verify Build 0051 implementation ancestry"
    Write-Host "Verified prior checkpoint HEAD: $ExpectedHead"
    Write-Host "Verified Build 0051 implementation commit: $PriorCommit"
    ...
    if ($RegressionReports.Count -eq 0) {
        throw "No passed Build 0051 Windows PowerShell 5.1 pre-release regression report was found."
    }
    ...
    if ($RegressionResult.status -ne "PASS") {
        throw "The latest Build 0051 Windows PowerShell 5.1 regression report is not PASS."
    }
    if (-not [string]::Equals(
        ...
        throw "The passed Build 0051 regression report does not match the selected package SHA-256."
    }
    ...
    $DryRunReport = Join-Path $ReportRoot "BUILD_0051_DRY_RUN_$Stamp.json"
    $ApplyReport = Join-Path $ReportRoot "BUILD_0051_APPLY_$Stamp.json"
    $ValidatorReport = Join-Path $ReportRoot "BUILD_0051_VALIDATOR_$Stamp.json"
    $RollbackReport = Join-Path $ReportRoot "BUILD_0051_ROLLBACK_$Stamp.json"

    Write-Step "Extract controlled launchers"
    $TempRoot = Join-Path $env:TEMP ("Certiaura_0051_QuickRun_" + [guid]::NewGuid().ToString("N"))
    Expand-Archive -LiteralPath $Package -DestinationPath $TempRoot -Force
    $Preflight = Join-Path $TempRoot "Scripts\Invoke_Certiaura_Build_0051_Preflight.ps1"
    $Importer = Join-Path $TempRoot "Scripts\Invoke_Certiaura_Build_0051_Import.ps1"
    $RollbackHelper = Join-Path $TempRoot "13_Project_Genesis\Import\rollback_build_0051_pending_import.py"
    ...
    if ($LASTEXITCODE -ne 0) { throw "Build 0051 preflight failed." }

    Write-Step "Run Project Genesis dry-run"
    & $Importer -Repository $Repository -Package $Package -Report $DryRunReport
    if ($LASTEXITCODE -ne 0) { throw "Build 0051 dry-run failed." }
    ...
    if ($LASTEXITCODE -ne 0) { throw "Build 0051 apply failed." }
    ...
    Write-Step "Run Build 0051 validator and tests"
    $Python = Resolve-Python
    $env:PYTHONDONTWRITEBYTECODE = "1"
    & $Python -B (Join-Path $Repository "13_Project_Genesis\Validators\validate_retatrutide_post_closure_surveillance_reopening.py") $Repository --report $ValidatorReport
    Assert-Exit "Build 0051 validator"
    ...
    Assert-Exit "Build 0051 regression tests"
    ...
    if ($CommitConfirmation -cne "COMMIT") { Write-Host "Build 0051 is staged but not committed." -ForegroundColor Yellow; Write-Host $CommitMessage; return }
    git -C $Repository commit -m $CommitMessage
    Assert-Exit "Git commit"
    $CommitCompleted = $true
    git -C $Repository push origin main
    Assert-Exit "Git push"
    Write-Host "BUILD 0051 COMMITTED AND PUSHED" -ForegroundColor Green
    ...
    if ($ApplyCompleted -and -not $CommitCompleted) {
        ...
        try {
            if (-not $RollbackHelper -or -not (Test-Path -LiteralPath $RollbackHelper -PathType Leaf)) {
                throw "Build 0051 rollback helper is unavailable."
            }
            if (-not $ApplyReport -or -not (Test-Path -LiteralPath $ApplyReport -PathType Leaf)) {
                throw "Build 0051 apply report is unavailable for rollback."
            }
            ...
            if ($LASTEXITCODE -ne 0) {
                throw "Build 0051 post-apply rollback failed with exit code $LASTEXITCODE."
            }
            ...
            if (-not [bool]$RollbackResult.valid -or $RollbackResult.status -ne "ROLLED_BACK_CLEAN") {
                throw "Build 0051 rollback report is not clean."
            }
            $ApplyCompleted = $false
            Write-Host "BUILD 0051 POST-APPLY ROLLBACK: PASS" -ForegroundColor Green
            Write-Host "Rollback report: $RollbackReport"
        }
        catch {
            Write-Host "BUILD 0051 POST-APPLY ROLLBACK: FAILED" -ForegroundColor Red
            Write-Host $_.Exception.Message -ForegroundColor Red
            Write-Host "External backup and apply report have been retained for controlled recovery." -ForegroundColor Yellow
        }
    }
    Write-Host "BUILD 0051 STOPPED - FAIL-CLOSED" -ForegroundColor Red
