...
$ExpectedHead = "2bf6c2f9fcaacfc0d0942045178269a1241253ee"
$ExpectedCommit = "Add Certiaura Build 0058 tesamorelin multi-source evidence quality assessment, conflicting-evidence adjudication, longitudinal signal recurrence, controlled amendment and pilot continuation governance baseline"
...
function Assert-OptionalManifestPropertyHandling {
    ...
    Write-Host "BUILD_0058_OPTIONAL_PROPERTY_STRICTMODE_VALIDATED" -ForegroundColor Green
}
...
if ($Hash -ne $ExpectedPackageSha256) {
    throw "Build 0058 package SHA-256 mismatch. Expected $ExpectedPackageSha256; observed $Hash."
}

$InitialStatus = @(git -C $Repository status --porcelain)
Assert-Exit "read initial repository status"
if ($InitialStatus.Count -gt 0) {
    throw "Repository must be clean before Build 0058."
}
...
if ($Head -ne $ExpectedHead) {
    throw "Repository HEAD $Head does not equal closed Build 0057 commit $ExpectedHead."
}

$Temp = Join-Path $env:TEMP ("CERTIAURA_0058_" + [guid]::NewGuid().ToString("N"))
...
$CanonicalImportReport = Join-Path $ReportRoot "BUILD_0058_CANONICAL_IMPORT.json"
$AutomaticRollbackReport = Join-Path $ReportRoot "BUILD_0058_AUTOMATIC_ROLLBACK.json"

try {
    New-Item -ItemType Directory -Path $Extract -Force | Out-Null
    Expand-Archive -LiteralPath $PackagePath -DestinationPath $Extract -Force

    $Preflight = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0058_Preflight.ps1"
    $Regression = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0058_Windows_PS51_Regression.ps1"
    $Importer = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0058_Import.ps1"

    & $Preflight `
        -PackagePath $PackagePath `
        -ReportPath (Join-Path $ReportRoot "BUILD_0058_PREFLIGHT.json")

    & $Importer `
        -PackagePath $PackagePath `
        -Repository $Repository `
        -BackupRoot $BackupRoot `
        -ReportPath (Join-Path $ReportRoot "BUILD_0058_CANONICAL_DRY_RUN.json")
    ...
    $Validator = Join-Path $Repository "13_Project_Genesis\Validators\validate_build_0058_repository.py"
    $ManifestPath = Join-Path $Repository "Documentation\Build_Records\0058\ASSET_INTENT_MANIFEST.json"
    $RepositoryReport = Join-Path $Repository "Documentation\Build_Records\0058\POST_IMPORT_REPOSITORY_VALIDATION.json"
    ...
    Assert-Exit "Build 0058 repository validation"
    ...
    Assert-Exit "stage Build 0058 owned paths"
    ...
    Assert-Exit "read staged Build 0058 paths"
    ...
    Assert-Exit "read actual Build 0058 change-set"
    ...
    Assert-Exit "read untracked paths after Build 0058 staging"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Changes exist outside Build 0058 Asset Intent Manifest ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    if ($UnexpectedNoOp.Count -ne 0) {
        throw (
            "Unexpected no-op Build 0058 owned paths: " +
            ($UnexpectedNoOp -join ", ")
        )
    }
    ...
    if ($Difference.Count -ne 0) {
        throw "Final staged change-set does not exactly equal the actual Build 0058 Git change-set."
    }

    Write-Host (
        "BUILD_0058_STAGED_CHANGESET_VALIDATED owned=" +
        $Owned.Count +
        " staged=" +
        $Observed.Count +
        " approved_no_op=" +
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot "BUILD_0058_STAGED_BYTE_EQUALITY.json")
    )
    & $Python.Path @ByteArguments
    Assert-Exit "raw staged byte equality"

    $CommitFile = Join-Path $Repository "Documentation\Build_Records\0058\COMMIT_MESSAGE.txt"
    if ((Get-Content -LiteralPath $CommitFile -Raw).Trim() -ne $ExpectedCommit) {
        throw "Reserved commit subject mismatch."
    }

    Write-Host ""
    Write-Host "BUILD_0058_CANONICAL_VALIDATED_AND_STAGED" -ForegroundColor Green
    ...
    if ($CanonicalApplied) {
        try {
            ...
            Write-Host "BUILD_0058_AUTOMATIC_ROLLBACK_STATE_EXACT" -ForegroundColor Yellow
        }
        catch {
            throw (
                "Build 0058 post-import gate failed and automatic rollback also failed. " +
