...
$TempRoot = Join-Path `
    $env:TEMP `
    ("CERTIAURA_0056_PS51_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $Importer = Join-Path `
        $Extract `
        "Scripts\Invoke_Certiaura_Build_0056_Import.ps1"

    $ForcedReport = Join-Path `
        $ReportRoot `
        ("BUILD_0056_FORCED_ROLLBACK_" + $Timestamp + ".json")
    ...
    $CleanReport = Join-Path `
        $ReportRoot `
        ("BUILD_0056_CLEAN_REAPPLY_" + $Timestamp + ".json")
    ...
    $RepositoryValidator = Join-Path `
        $Clone `
        "13_Project_Genesis\Validators\validate_build_0056_repository.py"
    $RepositoryReport = Join-Path `
        $Clone `
        "Documentation\Build_Records\0056\POST_IMPORT_REPOSITORY_VALIDATION.json"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone Build 0056 repository validation failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone full Build 0056 test suite failed before canonical apply."
    }

    Write-Host "BUILD_0056_PRE_CANONICAL_FULL_SUITE_VALIDATED" -ForegroundColor Green

    $ManifestPath = Join-Path `
        $Clone `
        "Documentation\Build_Records\0056\ASSET_INTENT_MANIFEST.json"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Temporary clone contains changes outside Build 0056 ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot ("BUILD_0056_PS51_BYTE_EQUALITY_" + $Timestamp + ".json"))
    )
    ...
    & git -C $Clone remote add build0056_regression $BareRemote
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary remote configuration failed."
    }
    & git -C $Clone push build0056_regression HEAD:refs/heads/build0056-regression
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary push-path regression failed."
    }

    Write-Host "BUILD_0056_WINDOWS_PS51_REGRESSION_PASS" -ForegroundColor Green
}
finally {
