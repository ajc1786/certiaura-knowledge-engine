...
$ExpectedHead = "011f5a47d756d638b4c0c8b2e122628ff5a6d35a"
...
if ($Hash -ne $ExpectedPackageSha256) {
    throw "Build 0059 package SHA-256 mismatch. Expected $ExpectedPackageSha256; observed $Hash."
}

$InitialStatus = @(git -C $Repository status --porcelain)
Assert-Exit "read initial repository status"
if ($InitialStatus.Count -gt 0) {
    throw "Repository must be clean before Build 0059."
}
...
if ($Head -ne $ExpectedHead) {
    throw "Repository HEAD $Head does not equal closed Build 0058 commit $ExpectedHead."
}

$Temp = Join-Path $env:TEMP ("CERTIAURA_0059_" + [guid]::NewGuid().ToString("N"))
...
$CanonicalImportReport = Join-Path $ReportRoot "BUILD_0059_CANONICAL_IMPORT.json"
$AutomaticRollbackReport = Join-Path $ReportRoot "BUILD_0059_AUTOMATIC_ROLLBACK.json"

try {
    New-Item -ItemType Directory -Path $Extract -Force | Out-Null
    Expand-Archive -LiteralPath $PackagePath -DestinationPath $Extract -Force

    $Preflight = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0059_Preflight.ps1"
    $Regression = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0059_Windows_PS51_Regression.ps1"
    $Importer = Join-Path $Extract "Scripts\Invoke_Certiaura_Build_0059_Import.ps1"

    & $Preflight `
        -PackagePath $PackagePath `
        -ReportPath (Join-Path $ReportRoot "BUILD_0059_PREFLIGHT.json")

    & $Importer `
        -PackagePath $PackagePath `
        -Repository $Repository `
        -BackupRoot $BackupRoot `
        -ReportPath (Join-Path $ReportRoot "BUILD_0059_CANONICAL_DRY_RUN.json")
    ...
    $Validator = Join-Path $Repository "13_Project_Genesis\Validators\validate_build_0059_repository.py"
    $ManifestPath = Join-Path $Repository "Documentation\Build_Records\0059\ASSET_INTENT_MANIFEST.json"
    $RepositoryReport = Join-Path $Repository "Documentation\Build_Records\0059\POST_IMPORT_REPOSITORY_VALIDATION.json"
    ...
    Assert-Exit "Build 0059 repository validation"
    ...
    Assert-Exit "stage Build 0059 owned paths"
    ...
    Assert-Exit "read staged Build 0059 paths"
    ...
    Assert-Exit "read actual Build 0059 change-set"
    ...
    Assert-Exit "read untracked paths after Build 0059 staging"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Changes exist outside Build 0059 Asset Intent Manifest ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    if ($UnexpectedNoOp.Count -ne 0) {
        throw (
            "Unexpected no-op Build 0059 owned paths: " +
            ($UnexpectedNoOp -join ", ")
        )
    }
    ...
    if ($Difference.Count -ne 0) {
        throw "Final staged change-set does not exactly equal the actual Build 0059 Git change-set."
    }
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot "BUILD_0059_STAGED_BYTE_EQUALITY.json")
    )
    & $Python.Path @ByteArguments
    Assert-Exit "raw staged byte equality"

    $CommitFile = Join-Path $Repository "Documentation\Build_Records\0059\COMMIT_MESSAGE.txt"
    if ((Get-Content -LiteralPath $CommitFile -Raw).Trim() -ne $ExpectedCommit) {
        throw "Reserved commit subject mismatch."
    }

    Write-Host ""
    Write-Host "BUILD_0059_CANONICAL_VALIDATED_AND_STAGED" -ForegroundColor Green
    ...
    if ($CanonicalApplied) {
        try {
            ...
            Write-Host "BUILD_0059_AUTOMATIC_ROLLBACK_STATE_EXACT" -ForegroundColor Yellow
        }
        catch {
            throw (
                "Build 0059 post-import gate failed and automatic rollback also failed. " +
