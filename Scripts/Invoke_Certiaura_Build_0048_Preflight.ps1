...
$TempRoot = Join-Path $env:TEMP ("Certiaura_0048_Preflight_" + [guid]::NewGuid().ToString("N"))
try {
    Expand-Archive -LiteralPath $Package -DestinationPath $TempRoot -Force
    $Python = Resolve-Python
    $PackageVerifier = Join-Path $TempRoot "13_Project_Genesis\Validators\verify_build_0048_package.py"
    $Validator = Join-Path $TempRoot "13_Project_Genesis\Validators\validate_retatrutide_clinician_approval_handoff.py"
    foreach ($Required in @($PackageVerifier, $Validator)) { if (-not (Test-Path -LiteralPath $Required -PathType Leaf)) { throw "Build 0048 validation component is missing: $Required" } }
    $env:PYTHONDONTWRITEBYTECODE = "1"
    & $Python -B $PackageVerifier $Package
    if ($LASTEXITCODE -ne 0) { throw "Build 0048 package checksum and hygiene verification failed." }
    & $Python -B $Validator $TempRoot --report $Report
    if ($LASTEXITCODE -ne 0) { throw "Build 0048 package validation failed." }
    ...
    Write-Host "Build 0048 preflight passed." -ForegroundColor Green
}
finally {
