...
$TempRoot = Join-Path `
    $env:TEMP `
    ("CERTIAURA_0057_PS51_" + [guid]::NewGuid().ToString("N"))
...
try {
    ...
    $Importer = Join-Path `
        $Extract `
        "Scripts\Invoke_Certiaura_Build_0057_Import.ps1"

    $ForcedReport = Join-Path `
        $ReportRoot `
        ("BUILD_0057_FORCED_ROLLBACK_" + $Timestamp + ".json")
    ...
    $CleanReport = Join-Path `
        $ReportRoot `
        ("BUILD_0057_CLEAN_REAPPLY_" + $Timestamp + ".json")
    ...
    $RepositoryValidator = Join-Path `
        $Clone `
        "13_Project_Genesis\Validators\validate_build_0057_repository.py"
    $RepositoryReport = Join-Path `
        $Clone `
        "Documentation\Build_Records\0057\POST_IMPORT_REPOSITORY_VALIDATION.json"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone Build 0057 repository validation failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone full Build 0057 test suite failed before canonical apply."
    }

    Write-Host "BUILD_0057_PRE_CANONICAL_FULL_SUITE_VALIDATED" -ForegroundColor Green

    $ManifestPath = Join-Path `
        $Clone `
        "Documentation\Build_Records\0057\ASSET_INTENT_MANIFEST.json"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Temporary clone contains changes outside Build 0057 ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot ("BUILD_0057_PS51_BYTE_EQUALITY_" + $Timestamp + ".json"))
    )
    ...
    Write-Host "BUILD_0057_GIT_NONINTERACTIVE_GUARD_VALIDATED" -ForegroundColor Green
    ...
    & git -C $Clone remote add build0057_regression $BareRemote
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary remote configuration failed."
    }
    Invoke-CertiauraGitNonInteractiveGuard -Repository $Clone -ScriptBlock {
        & git -C $Clone push build0057_regression HEAD:refs/heads/build0057-regression
        if ($LASTEXITCODE -ne 0) { throw "Temporary push-path regression failed." }
    }

    Write-Host "BUILD_0057_WINDOWS_PS51_REGRESSION_PASS" -ForegroundColor Green
}
finally {
