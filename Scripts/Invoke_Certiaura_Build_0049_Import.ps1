...
$TempRoot = Join-Path $env:TEMP ("Certiaura_0049_Import_" + [guid]::NewGuid().ToString("N"))
try {
    Expand-Archive -LiteralPath $Package -DestinationPath $TempRoot -Force
    $Importer = Join-Path $TempRoot "13_Project_Genesis\Import\build_0049_transactional_import.py"
    if (-not (Test-Path -LiteralPath $Importer -PathType Leaf)) { throw "Build 0049 importer is missing." }
    ...
    & $Python @Arguments
    if ($LASTEXITCODE -ne 0) { throw "Build 0049 transactional importer failed." }
}
finally {
