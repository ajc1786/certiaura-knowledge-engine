...
param(
    ...
    [string]$HistoricalAuditDirectory,
    [switch]$Apply,
...
$Script = Join-Path $PSScriptRoot "..\13_Project_Genesis\Import\run_build_0055_import.py"
$Arguments = @($Prefix) + @("-B", $Script, "--package", $PackagePath, "--repository", $Repository, "--backup-root", $BackupRoot, "--report", $ReportPath)
if (-not [string]::IsNullOrWhiteSpace($HistoricalAuditDirectory)) { $Arguments += @("--historical-audit-directory", $HistoricalAuditDirectory) }
...
if ($LASTEXITCODE -ne 0) { throw "Build 0055 import failed with exit code $LASTEXITCODE." }
