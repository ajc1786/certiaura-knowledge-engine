...
param(
    ...
    [Parameter(Mandatory = $true)][string]$HistoricalAuditDirectory
)
...
$TempRoot = Join-Path `
    $env:TEMP `
    ("CERTIAURA_0055_PS51_" + [guid]::NewGuid().ToString("N"))
...
if (-not (Test-Path -LiteralPath $HistoricalAuditDirectory -PathType Container)) {
    throw "Historical Actions audit directory not found: $HistoricalAuditDirectory"
}
...
try {
    ...
    $Importer = Join-Path `
        $Extract `
        "Scripts\Invoke_Certiaura_Build_0055_Import.ps1"

    $ForcedReport = Join-Path `
        $ReportRoot `
        ("BUILD_0055_FORCED_ROLLBACK_" + $Timestamp + ".json")
    ...
    $CleanReport = Join-Path `
        $ReportRoot `
        ("BUILD_0055_CLEAN_REAPPLY_" + $Timestamp + ".json")
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone Build 0055 repository validation failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary clone full Build 0055 test suite failed before canonical apply."
    }

    Write-Host "BUILD_0055_PRE_CANONICAL_FULL_SUITE_VALIDATED" -ForegroundColor Green

    $ManifestPath = Join-Path `
        $Clone `
        "Documentation\Build_Records\0055\ASSET_INTENT_MANIFEST.json"
    ...
    if ($OutsideOwned.Count -ne 0) {
        throw (
            "Temporary clone contains changes outside Build 0055 ownership: " +
            ($OutsideOwned -join ", ")
        )
    }
    ...
    $ByteArguments = @($Python.Prefix) + @(
        ...
        $ManifestPath,
        "--report",
        (Join-Path $ReportRoot ("BUILD_0055_PS51_BYTE_EQUALITY_" + $Timestamp + ".json"))
    )
    ...
    & git -C $Clone remote add build0055_regression $BareRemote
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary remote configuration failed."
    }
    & git -C $Clone push build0055_regression HEAD:refs/heads/build0055-regression
    if ($LASTEXITCODE -ne 0) {
        throw "Temporary push-path regression failed."
    }

    Write-Host "BUILD_0055_WINDOWS_PS51_REGRESSION_PASS" -ForegroundColor Green
}
finally {
