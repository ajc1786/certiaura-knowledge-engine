...
BUILD="0046"; PROVENANCE="CERT-BUILD-0046"; MANIFEST="Documentation/Build_Records/0046/ASSET_INTENT_MANIFEST.json"
...
def register_candidates(repository):
 return [p for p in repository.rglob("Master_Asset_Register.csv") if ".git" not in p.parts and ".certiaura_backups" not in p.parts]
 ...
 if a.apply and not errors and not conflicts:
  if not a.backup_root: errors.append("External backup root is required")
   ...
   if inside(br,repo): errors.append("Backup root must be outside canonical repository")
    ...
    backup=br/("Build_0046_Pre_Import_"+datetime.now().strftime("%Y%m%d_%H%M%S")); journal=[]; original=register.read_bytes()
    try:
     ...
     for alloc in allocations:
      ...
      row={c:"" for c in fields}; row.update({"Universal Asset Identifier":alloc["uai"],"Asset Name":asset["asset_title"],"Knowledge System":asset["knowledge_system"],"Asset Type":asset["asset_type"],"Status":asset["proposed_status"],"Owner":asset["owner"],"Parent Assets":"; ".join(asset.get("parent_assets",[])),"Notes":"Created by Certiaura Build 0046.","Repository Path":target,"Supporting Files":"; ".join(asset.get("supporting_files",[])),"Version":asset["proposed_version"],"Completion Percentage":str(asset.get("completion_percentage",0)),"Child Assets":"; ".join(asset.get("child_assets",[])),"Relationship List":json.dumps(asset.get("relationships",[]),separators=(",",":")),"Evidence Links":"; ".join(asset.get("evidence_links",[])),"Report Links":"; ".join(asset.get("report_links",[])),"Marketplace Links":"; ".join(asset.get("marketplace_links",[])),"Change History":"Created by Build 0046","Build Provenance":PROVENANCE,"Source Builds":"0046","Registration Basis":"Asset Intent Manifest transactional import","File SHA256":hfile(dest),"Last Updated":now()}); rows.append(row)
