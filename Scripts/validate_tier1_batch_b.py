...
closure = rows("Documentation/Audit/Tier1_Batch_B_Source_Closure_Register.csv")
evidence = rows("Documentation/Audit/Tier1_Batch_B_Evidence_Register.csv")
gaps = rows("Documentation/Audit/Tier1_Batch_B_Platinum_Gap_Register.csv")

new_passports = {
    row["Product Passport"]
    for row in closure
    if row["Passport Action"] == "NEW"
}
patched_passports = {
    row["Product Passport"]
    for row in closure
    if row["Passport Action"] == "PATCH"
}
evidence_ids = {row["EKS UAI"] for row in evidence}

checks = {
    "seven_assets": len(closure) == 7,
    "six_new_passports": new_passports
    == {f"CERT-PPS-{number:06d}" for number in range(59, 65)},
    "elamipretide_passport_patched": patched_passports == {"CERT-PPS-000031"},
    "twenty_one_evidence": len(evidence) == 21,
    "contiguous_evidence": evidence_ids
    == {f"CERT-EKS-{number:06d}" for number in range(356, 377)},
...
print("BUILD 0035C VALIDATION: PASS")
