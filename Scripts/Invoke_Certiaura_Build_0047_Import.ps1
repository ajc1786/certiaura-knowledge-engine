...
$TempRoot = Join-Path $env:TEMP ("Certiaura_0047_Import_" + [guid]::NewGuid().ToString("N"))
try {
    Expand-Archive -LiteralPath $Package -DestinationPath $TempRoot -Force
    $Importer = Join-Path $TempRoot "13_Project_Genesis\Import\build_0047_transactional_import.py"
    if (-not (Test-Path -LiteralPath $Importer -PathType Leaf)) { throw "Build 0047 importer is missing." }
    ...
    & $Python @Arguments
    if ($LASTEXITCODE -ne 0) { throw "Build 0047 transactional importer failed." }
}
finally {
