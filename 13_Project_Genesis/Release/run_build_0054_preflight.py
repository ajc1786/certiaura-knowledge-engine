...
def main()->int:
 ...
 try:
  ...
  with tempfile.TemporaryDirectory() as td:
   ...
   manifest=json.loads((root/"Documentation/Build_Records/0054/ASSET_INTENT_MANIFEST.json").read_text(encoding="utf-8")); names=sorted(str(p.relative_to(root)).replace("\\","/") for p in root.rglob("*") if p.is_file()); declared=sorted(str(x["repository_path"]).replace("\\","/") for x in manifest["files"])
   ...
   validator=root/"13_Project_Genesis/Validators/validate_retatrutide_operational_assurance.py"
   ...
   tests=run([sys.executable,"-B","-m","unittest","discover","-s","13_Project_Genesis/Tests","-p","test_build_0054_*.py","-v"],cwd=root)
   checks["automated_tests"]="PASS"; checks["test_output"]=tests.stdout[-8000:]
 except Exception as exc: errors.append(str(exc))
 result={"build_number":"0054","candidate":"RC4","valid":not errors,"checks":checks,"errors":errors,"result":"CANDIDATE_RELEASE_VALIDATED" if not errors else "FAIL"}
