...
MANIFEST_PATH = "Documentation/Build_Records/0057/ASSET_INTENT_MANIFEST.json"
DELIVERY_PATH = "Documentation/Build_Records/0057/CANDIDATE_DELIVERY.json"
...
def derive(
    ...
    if str(predecessor.get("build_number")) != EXPECTED_BUILD:
        raise RuntimeError("predecessor manifest build_number is not 0057")
    if str(predecessor.get("candidate")) != EXPECTED_CANDIDATE:
        raise RuntimeError("predecessor candidate is not RC1")
    ...
    if delivery_result.returncode == 0:
        ...
        if observed_candidate and observed_candidate != EXPECTED_CANDIDATE:
            raise RuntimeError("Build 0057 delivery evidence does not identify RC1")

    result = {
        "schema_version": "1.0.0",
        "generated_at": datetime.now(timezone.utc).isoformat(),
        "source": "CANONICAL_GIT_OBJECTS",
        "predecessor_build": "0057",
