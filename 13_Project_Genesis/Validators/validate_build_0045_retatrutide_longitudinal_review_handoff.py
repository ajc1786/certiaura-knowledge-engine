...
BUILD="0045"; PROV="CERT-BUILD-0045"; REGISTER="Documentation/Master_Asset_Register.csv"; INTENT="Documentation/Build_Records/0045/ASSET_INTENT_MANIFEST.json"
DEPS=["Scripts/generate_retatrutide_patient_journey_report.py","Scripts/query_retatrutide_knowledge.py","Scripts/render_retatrutide_branded_report.py","Scripts/run_retatrutide_controlled_conversation.py","12_Reports/Retatrutide/Patient_Interface/CERT-RKS-000004_Retatrutide_Patient_Facing_Interface_Baseline.md","13_Project_Genesis/AI/CERT-SYS-000851_Retatrutide_Controlled_Conversation_Workflow_Baseline.md"]
TEXT={".md",".json",".csv",".py",".ps1",".cmd",".bat",".txt",".html",".css",".js",".yml",".yaml"}
def sha(path): return hashlib.sha256(path.read_bytes()).hexdigest()
def rows(repo):
 p=repo/REGISTER
 if not p.is_file(): return []
 with p.open("r",encoding="utf-8-sig",newline="") as f: return list(csv.DictReader(f))
def owned(repo,register_rows):
 p=repo/INTENT
 if not p.is_file(): return []
...
def hygiene(repo,rel):
 ...
 if not p.is_file(): return ["Build 0045-owned file missing: "+rel]
 data=p.read_bytes()
...
def main():
 ...
 canonical=repo/REGISTER; candidates=[x for x in repo.rglob("Master_Asset_Register.csv") if ".git" not in x.parts]
 ...
 if len(build_rows)!=3: errors.append(f"Expected 3 Build 0045 formal assets; found {len(build_rows)}")
 ...
 for path in repo.rglob("*"):
  ...
  if "__UAI__" in path.name:
   errors.append("Unresolved UAI filename placeholder: "+relative)
  if (relative.startswith("05_Monitoring/Retatrutide/Longitudinal_Journey/") or relative.startswith("05_Monitoring/Retatrutide/Review_Scheduling/") or relative.startswith("12_Reports/Retatrutide/Clinician_Handoff/")) and path.suffix.lower()==".md" and b"UAI_ALLOCATION_REQUIRED" in path.read_bytes():
   errors.append("Unresolved formal asset content placeholder: "+relative)
 ui=repo/"13_Project_Genesis/UI/Retatrutide_Longitudinal_Journey"
