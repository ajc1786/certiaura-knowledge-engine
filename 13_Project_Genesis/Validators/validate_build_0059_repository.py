...
from build_0059_asset_ownership import load_manifest,owned_paths
...
CHECKPOINT_MARKER="<!-- CERTIAURA_BUILD_0059_CHECKPOINT_START -->"
LESSONS_MARKER="<!-- CERTIAURA_BUILD_0059_LESSONS_START -->"
def normalise(v): return str(v).replace("\\","/")
def validate(repository,report_path=None,expected_predecessor_commit=EXPECTED_COMMIT):
 repo=Path(repository).resolve(); errors=[]; mp=repo/"Documentation/Build_Records/0059/ASSET_INTENT_MANIFEST.json"
 if not mp.exists(): return {"build_number":"0059","valid":False,"errors":["Build 0059 Asset Intent Manifest missing"],"result":"FAIL"}
 ...
 if len(ex)!=20: errors.append("expected 20 examples")
 ...
 close=repo/"Scripts/Close_Certiaura_Build_0059.ps1"
 if close.exists():
  text=close.read_text(encoding="utf-8")
  tokens=["Invoke-CertiauraGitNonInteractiveGuard","CERTIAURA_BUILD_0059_CLOSURE_EVIDENCE_BEGIN","CERTIAURA_BUILD_0059_CLOSURE_EVIDENCE_END","BUILD_0059_COMMITTED_PUSHED","BUILD_0059_GITHUB_ACTIONS_GREEN","BUILD_0059_READY_FOR_FOUNDER_GREEN","NO_MANUAL_GIT_OBJECT_CLEANUP_PROMPTS","BUILD_0059_CLOSURE_EVIDENCE.json","Actions URL:","Run attempt:","Local and origin/main aligned:","Repository clean:"]
  for token in tokens:
   if token not in text: errors.append("close script missing "+token)
 ignored=repo/"Documentation/Build_Records/0059/Closure_Evidence/.gitignore"
 ...
 result={"build_number":"0059","owned_path_count":len(owned_paths(m)),"example_count":len(ex),"formal_asset_count":len(formal),"valid":not errors,"errors":errors,"result":"BUILD_0059_REPOSITORY_VALIDATED" if not errors else "FAIL"}
