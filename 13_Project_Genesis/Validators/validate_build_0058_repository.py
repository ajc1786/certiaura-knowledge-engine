...
from build_0058_asset_ownership import load_manifest,owned_paths
from tesamorelin_longitudinal_review_common import load_json,validate_record
EXPECTED_COMMIT="2bf6c2f9fcaacfc0d0942045178269a1241253ee"
CHECKPOINT_MARKER="<!-- CERTIAURA_BUILD_0058_CHECKPOINT_START -->"
LESSONS_MARKER="<!-- CERTIAURA_BUILD_0058_LESSONS_START -->"
def normalise(v): return str(v).replace("\\","/")
def validate(repository,report_path=None,expected_predecessor_commit=EXPECTED_COMMIT):
 repo=Path(repository).resolve(); errors=[]; mp=repo/"Documentation/Build_Records/0058/ASSET_INTENT_MANIFEST.json"
 if not mp.exists(): return {"build_number":"0058","valid":False,"errors":["Build 0058 Asset Intent Manifest missing"],"result":"FAIL"}
 ...
 if len(ex)!=16: errors.append("expected 16 examples")
 formal=[i for i in m.get("files",[]) if i.get("classification")=="FORMAL_ASSET"]
 if len(formal)!=6: errors.append("expected 6 formal assets")
 ...
 close=(repo/"Scripts/Close_Certiaura_Build_0058.ps1")
 if close.exists():
  text=close.read_text(encoding="utf-8")
  for token in ["Invoke-CertiauraGitNonInteractiveGuard","BUILD_0058_COMMITTED_PUSHED","BUILD_0058_GITHUB_ACTIONS_GREEN","NO_MANUAL_GIT_OBJECT_CLEANUP_PROMPTS"]:
   if token not in text: errors.append("close script missing "+token)
 result={"build_number":"0058","owned_path_count":len(owned_paths(m)),"example_count":len(ex),"formal_asset_count":len(formal),"valid":not errors,"errors":errors,"result":"BUILD_0058_REPOSITORY_VALIDATED" if not errors else "FAIL"}
