...
PACKAGE = Path("/mnt/data/Certiaura_Build_0056_Tesamorelin_Controlled_Pilot_RC1.zip")
PREDECESSOR = Path("/mnt/data/Certiaura_Build_0055_Historical_Actions_Reconciliation_RC2.zip")
IMPORTER_PATH = ROOT / "13_Project_Genesis/Import/run_build_0056_import.py"
VALIDATOR_PATH = ROOT / "13_Project_Genesis/Validators/validate_build_0056_repository.py"
...
class Tests(unittest.TestCase):
    ...
    def test_rollback_clean_reapply_and_validator(self):
        importer = load_module(IMPORTER_PATH, "import0056")
        validator = load_module(VALIDATOR_PATH, "validate0056")
    ...
    def test_generated_reports_are_lf(self):
        importer = load_module(IMPORTER_PATH, "import0056_lf")
        with tempfile.TemporaryDirectory() as temp:
            repo = Path(temp) / "repo"
            repo.mkdir()
            commit = seed_repository(repo)
            applied = importer.run_import(PACKAGE, repo, Path(temp) / "backup", Path(temp) / "apply.json", True, expected_predecessor_commit=commit)
            manifest = json.loads((repo / "Documentation/Build_Records/0056/ASSET_INTENT_MANIFEST.json").read_text(encoding="utf-8"))
