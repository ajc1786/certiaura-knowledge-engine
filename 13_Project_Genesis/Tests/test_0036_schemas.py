...
class TestSchemas(unittest.TestCase):
 ...
 def test_manifest_commit(self): self.assertTrue(json.loads((ROOT/'Documentation'/'Build_Records'/'0036'/'BUILD_MANIFEST.json').read_text())['commit_message'].startswith('Add Certiaura Build 0036'))
 def test_no_external_packages(self): self.assertEqual(json.loads((ROOT/'Documentation'/'Build_Records'/'0036'/'BUILD_MANIFEST.json').read_text())['external_python_packages'],[])
