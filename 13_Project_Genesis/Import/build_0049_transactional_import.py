...
BUILD_ID = "CERT-BUILD-0049"
BUILD_NUMBER = "0049"
MANIFEST_PATH = "Documentation/Build_Records/0049/ASSET_INTENT_MANIFEST.json"
...
def reconcile_register(fields: list[str], rows: list[dict[str, str]], manifest: dict, stage: Path) -> tuple[list[dict[str, str]], list[dict]]:
    ...
    for item in manifest.get("files", []):
        ...
        row.update({
            ...
            "Registration Basis": item.get("registration_basis", "BUILD_0049_ASSET_INTENT_MANIFEST_EXACT_PATH"),
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="certiaura_0049_") as tmp:
            stage = Path(tmp)
            ...
            if args.apply:
                ...
                backup_path = backup_root / f"Build_0049_Pre_Import_{stamp}"
