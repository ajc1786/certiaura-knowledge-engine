...
BUILD_ID = "CERT-BUILD-0050"
BUILD_NUMBER = "0050"
MANIFEST_PATH = "Documentation/Build_Records/0050/ASSET_INTENT_MANIFEST.json"
...
def reconcile_register(fields: list[str], rows: list[dict[str, str]], manifest: dict, stage: Path) -> tuple[list[dict[str, str]], list[dict]]:
    ...
    for item in manifest.get("files", []):
        ...
        row.update({
            ...
            "Registration Basis": item.get("registration_basis", "BUILD_0050_ASSET_INTENT_MANIFEST_EXACT_PATH"),
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="certiaura_0050_") as tmp:
            stage = Path(tmp)
            ...
            if args.apply:
                ...
                backup_path = backup_root / f"Build_0050_Pre_Import_{stamp}"
