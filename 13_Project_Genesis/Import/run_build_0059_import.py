...
BUILD_NUMBER = "0059"
EXPECTED_PREDECESSOR_COMMIT = "011f5a47d756d638b4c0c8b2e122628ff5a6d35a"
ALLOWED_ROOTS = {
    "00_Governance",
...
CURRENT_MANIFEST = (
    PACKAGE_ROOT / "Documentation/Build_Records/0059/ASSET_INTENT_MANIFEST.json"
)
...
def load_manifest_from_zip(zf: zipfile.ZipFile) -> dict[str, Any]:
    path = "Documentation/Build_Records/0059/ASSET_INTENT_MANIFEST.json"
...
def validate_package(package: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    with zipfile.ZipFile(package, "r") as zf:
        ...
        checksum_name = "Documentation/Build_Records/0059/CHECKSUMS.sha256"
...
def load_predecessor_module():
    path = (
        PACKAGE_ROOT
        / "13_Project_Genesis/Release/derive_build_0058_predecessor_evidence.py"
    )
    spec = importlib.util.spec_from_file_location("build0058_predecessor", path)
...
def run_import(
    ...
    if git(repo, "status", "--porcelain").stdout.strip():
        raise RuntimeError("repository must be clean before Build 0059 import")
    ...
    backup = backup_root_path / f"Build_0059_Pre_Import_{timestamp}"
    ...
    created: list[str] = []
    predecessor_rel = (
        "Documentation/Build_Records/0059/PREDECESSOR_CANONICAL_EVIDENCE.json"
    )
    register_report_rel = (
        "Documentation/Build_Records/0059/ASSET_REGISTER_CHANGE_REPORT.json"
    )
    import_report_rel = (
        "Documentation/Build_Records/0059/CANONICAL_IMPORT_REPORT.json"
    )
    ...
    try:
        ...
        updater = repo / "Scripts/update_certiaura_build_0059_controls.py"
...
def rollback_from_backup(
    ...
    if str(metadata.get("build_number")) != BUILD_NUMBER:
        raise RuntimeError("transaction metadata does not belong to Build 0059")
    ...
    if unexpected:
        raise RuntimeError(
            "rollback blocked by changes outside Build 0059 ownership: "
            + "; ".join(unexpected)
        )
