...
BUILD_NUMBER = "0060"
EXPECTED_PREDECESSOR_COMMIT = "594152fcfba3b1612b71d7b6e5c23759c906e464"
ALLOWED_ROOTS = {
    "00_Governance",
...
CURRENT_MANIFEST = (
    PACKAGE_ROOT / "Documentation/Build_Records/0060/ASSET_INTENT_MANIFEST.json"
)
...
def load_manifest_from_zip(zf: zipfile.ZipFile) -> dict[str, Any]:
    path = "Documentation/Build_Records/0060/ASSET_INTENT_MANIFEST.json"
...
def validate_package(package: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    with zipfile.ZipFile(package, "r") as zf:
        ...
        checksum_name = "Documentation/Build_Records/0060/CHECKSUMS.sha256"
...
def load_predecessor_module():
    path = (
        PACKAGE_ROOT
        / "13_Project_Genesis/Release/derive_build_0059_predecessor_evidence.py"
    )
    spec = importlib.util.spec_from_file_location("build0059_predecessor", path)
...
def run_import(
    ...
    if git(repo, "status", "--porcelain").stdout.strip():
        raise RuntimeError("repository must be clean before Build 0060 import")
    ...
    backup = backup_root_path / f"Build_0060_Pre_Import_{timestamp}"
    ...
    created: list[str] = []
    predecessor_rel = (
        "Documentation/Build_Records/0060/PREDECESSOR_CANONICAL_EVIDENCE.json"
    )
    register_report_rel = (
        "Documentation/Build_Records/0060/ASSET_REGISTER_CHANGE_REPORT.json"
    )
    import_report_rel = (
        "Documentation/Build_Records/0060/CANONICAL_IMPORT_REPORT.json"
    )
    validation_report_rel = (
        "Documentation/Build_Records/0060/POST_IMPORT_REPOSITORY_VALIDATION.json"
    )
    ...
    try:
        ...
        updater = repo / "Scripts/update_certiaura_build_0060_controls.py"
        ...
        repository_validator = (
            repo
            / "13_Project_Genesis/Validators/validate_build_0060_repository.py"
        )
...
def rollback_from_backup(
    ...
    if str(metadata.get("build_number")) != BUILD_NUMBER:
        raise RuntimeError("transaction metadata does not belong to Build 0060")
    ...
    if unexpected:
        raise RuntimeError(
            "rollback blocked by changes outside Build 0060 ownership: "
            + "; ".join(unexpected)
        )
