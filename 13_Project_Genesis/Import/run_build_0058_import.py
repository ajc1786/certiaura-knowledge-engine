...
BUILD_NUMBER = "0058"
EXPECTED_PREDECESSOR_COMMIT = "2bf6c2f9fcaacfc0d0942045178269a1241253ee"
ALLOWED_ROOTS = {
    "00_Governance",
...
CURRENT_MANIFEST = (
    PACKAGE_ROOT / "Documentation/Build_Records/0058/ASSET_INTENT_MANIFEST.json"
)
...
def load_manifest_from_zip(zf: zipfile.ZipFile) -> dict[str, Any]:
    path = "Documentation/Build_Records/0058/ASSET_INTENT_MANIFEST.json"
...
def validate_package(package: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    with zipfile.ZipFile(package, "r") as zf:
        ...
        checksum_name = "Documentation/Build_Records/0058/CHECKSUMS.sha256"
...
def load_predecessor_module():
    path = (
        PACKAGE_ROOT
        / "13_Project_Genesis/Release/derive_build_0057_predecessor_evidence.py"
    )
    spec = importlib.util.spec_from_file_location("build0057_predecessor", path)
...
def run_import(
    ...
    if git(repo, "status", "--porcelain").stdout.strip():
        raise RuntimeError("repository must be clean before Build 0058 import")
    ...
    backup = backup_root_path / f"Build_0058_Pre_Import_{timestamp}"
    ...
    created: list[str] = []
    predecessor_rel = (
        "Documentation/Build_Records/0058/PREDECESSOR_CANONICAL_EVIDENCE.json"
    )
    register_report_rel = (
        "Documentation/Build_Records/0058/ASSET_REGISTER_CHANGE_REPORT.json"
    )
    import_report_rel = (
        "Documentation/Build_Records/0058/CANONICAL_IMPORT_REPORT.json"
    )
    ...
    try:
        ...
        updater = repo / "Scripts/update_certiaura_build_0058_controls.py"
...
def rollback_from_backup(
    ...
    if str(metadata.get("build_number")) != BUILD_NUMBER:
        raise RuntimeError("transaction metadata does not belong to Build 0058")
    ...
    if unexpected:
        raise RuntimeError(
            "rollback blocked by changes outside Build 0058 ownership: "
            + "; ".join(unexpected)
        )
