...
BUILD_NUMBER = "0055"
EXPECTED_PREDECESSOR_COMMIT = "8a9dba6615448c1ad85c44cdf89269a8fc3e00e8"
ALLOWED_ROOTS = {
    "00_Governance",
...
CURRENT_MANIFEST = (
    PACKAGE_ROOT / "Documentation/Build_Records/0055/ASSET_INTENT_MANIFEST.json"
)

HISTORICAL_AUDIT_OUTPUTS = {
    "HISTORICAL_GITHUB_ACTIONS_EVIDENCE.json": "Documentation/Build_Records/0055/HISTORICAL_GITHUB_ACTIONS_EVIDENCE.json",
    "HISTORICAL_GITHUB_ACTIONS_EVIDENCE_REGISTER.csv": "Documentation/Build_Records/0055/HISTORICAL_GITHUB_ACTIONS_EVIDENCE_REGISTER.csv",
    "HISTORICAL_GITHUB_ACTIONS_EXCEPTIONS.json": "Documentation/Build_Records/0055/HISTORICAL_GITHUB_ACTIONS_EXCEPTIONS.json",
    "HISTORICAL_GITHUB_ACTIONS_AUDIT_SUMMARY.json": "Documentation/Build_Records/0055/HISTORICAL_GITHUB_ACTIONS_AUDIT_SUMMARY.json",
}
...
def load_manifest_from_zip(zf: zipfile.ZipFile) -> dict[str, Any]:
    path = "Documentation/Build_Records/0055/ASSET_INTENT_MANIFEST.json"
...
def validate_package(package: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    with zipfile.ZipFile(package, "r") as zf:
        ...
        checksum_name = "Documentation/Build_Records/0055/CHECKSUMS.sha256"
...
def validate_historical_audit_directory(path: str | Path) -> dict[str, Any]:
    root = Path(path).resolve()
    missing = [name for name in HISTORICAL_AUDIT_OUTPUTS if not (root / name).is_file()]
    if missing:
        raise RuntimeError("historical Actions audit files missing: " + ", ".join(missing))
    summary = json.loads((root / "HISTORICAL_GITHUB_ACTIONS_AUDIT_SUMMARY.json").read_text(encoding="utf-8"))
    evidence = json.loads((root / "HISTORICAL_GITHUB_ACTIONS_EVIDENCE.json").read_text(encoding="utf-8"))
    records = list(evidence.get("records", []))
    expected = [f"{number:04d}" for number in range(1, 55)]
    observed = [str(record.get("build_number", "")) for record in records]
    errors: list[str] = []
    if summary.get("result") != "HISTORICAL_ACTIONS_AUDIT_COMPLETE":
        errors.append("historical Actions audit result is not complete")
    if summary.get("expected_build_count") != 54:
        errors.append("historical Actions expected build count is not 54")
    if summary.get("record_count") != 54 or len(records) != 54:
        errors.append("historical Actions record count is not 54")
    if summary.get("accounted_count") != 54:
        errors.append("historical Actions accounted count is not 54")
    if summary.get("unresolved_count") != 0:
        errors.append("historical Actions unresolved count is not zero")
    if summary.get("all_builds_accounted") is not True:
        errors.append("historical Actions all_builds_accounted is not true")
    if observed != expected:
        errors.append("historical Actions build sequence is not exactly 0001-0054")
    if any(record.get("accounted") is not True for record in records):
        errors.append("one or more historical Actions records are unaccounted")
    evidence_summary = evidence.get("summary", {})
    if evidence_summary.get("result") != summary.get("result"):
        errors.append("historical Actions evidence and summary results differ")
    if errors:
...
def load_predecessor_module():
    path = (
        PACKAGE_ROOT
        / "13_Project_Genesis/Release/derive_build_0054_predecessor_evidence.py"
    )
    spec = importlib.util.spec_from_file_location("build0054_predecessor", path)
...
def run_import(
    ...
    historical_audit_directory: str | Path | None = None,
    ...
    if historical_audit_directory is None:
        raise RuntimeError("historical Actions audit directory is required")
    historical_audit_root = Path(historical_audit_directory).resolve()
    historical_audit_summary = validate_historical_audit_directory(historical_audit_root)
    ...
    if git(repo, "status", "--porcelain").stdout.strip():
        raise RuntimeError("repository must be clean before Build 0055 import")
    ...
    preflight = {
        ...
        "historical_actions_audit_result": historical_audit_summary.get("result"),
        "historical_actions_accounted_count": historical_audit_summary.get("accounted_count"),
        "historical_actions_verified_run_id_count": historical_audit_summary.get("verified_run_id_count"),
        "historical_actions_exception_count": historical_audit_summary.get("exception_count"),
        "historical_actions_all_exact_run_ids_captured": historical_audit_summary.get("all_exact_run_ids_captured"),
    ...
    backup = backup_root_path / f"Build_0055_Pre_Import_{timestamp}"
    ...
    created: list[str] = []
    predecessor_rel = (
        "Documentation/Build_Records/0055/PREDECESSOR_CANONICAL_EVIDENCE.json"
    )
    register_report_rel = (
        "Documentation/Build_Records/0055/ASSET_REGISTER_CHANGE_REPORT.json"
    )
    import_report_rel = (
        "Documentation/Build_Records/0055/CANONICAL_IMPORT_REPORT.json"
    )
    validation_report_rel = (
        "Documentation/Build_Records/0055/POST_IMPORT_REPOSITORY_VALIDATION.json"
    )
    ...
    try:
        ...
        for source_name, rel in HISTORICAL_AUDIT_OUTPUTS.items():
            source = historical_audit_root / source_name
            target = repo / rel
            target.parent.mkdir(parents=True, exist_ok=True)
        ...
        updater = repo / "Scripts/update_certiaura_build_0055_controls.py"
        ...
        repository_validator = (
            repo
            / "13_Project_Genesis/Validators/validate_build_0055_repository.py"
        )
...
def rollback_from_backup(
    ...
    if str(metadata.get("build_number")) != BUILD_NUMBER:
        raise RuntimeError("transaction metadata does not belong to Build 0055")
    ...
    if unexpected:
        raise RuntimeError(
            "rollback blocked by changes outside Build 0055 ownership: "
            + "; ".join(unexpected)
        )
...
def main() -> int:
    ...
    parser.add_argument("--historical-audit-directory")
    ...
    try:
        if args.rollback_backup:
            ...
            result = run_import(
                ...
                args.historical_audit_directory,
            )
