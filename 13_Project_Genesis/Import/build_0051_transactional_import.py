...
def reconcile_register(fields: list[str], rows: list[dict[str, str]], manifest: dict, stage: Path) -> tuple[list[dict[str, str]], list[dict]]:
    ...
    for item in manifest.get("files", []):
        ...
        row.update({
            ...
            "Registration Basis": item.get("registration_basis", "BUILD_0051_ASSET_INTENT_MANIFEST_EXACT_PATH"),
...
def main() -> int:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="certiaura_0051_") as tmp:
            stage = Path(tmp)
            ...
            if args.apply:
                ...
                backup_path = backup_root / f"Build_0051_Pre_Import_{stamp}"
