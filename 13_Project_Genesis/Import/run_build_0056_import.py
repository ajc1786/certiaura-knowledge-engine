...
BUILD_NUMBER = "0056"
EXPECTED_PREDECESSOR_COMMIT = "977829a987baf744beab4762478d9f0a88165fb0"
ALLOWED_ROOTS = {
    "00_Governance",
...
CURRENT_MANIFEST = (
    PACKAGE_ROOT / "Documentation/Build_Records/0056/ASSET_INTENT_MANIFEST.json"
)
...
def load_manifest_from_zip(zf: zipfile.ZipFile) -> dict[str, Any]:
    path = "Documentation/Build_Records/0056/ASSET_INTENT_MANIFEST.json"
...
def validate_package(package: Path) -> tuple[dict[str, Any], list[str]]:
    ...
    with zipfile.ZipFile(package, "r") as zf:
        ...
        checksum_name = "Documentation/Build_Records/0056/CHECKSUMS.sha256"
...
def load_predecessor_module():
    path = (
        PACKAGE_ROOT
        / "13_Project_Genesis/Release/derive_build_0055_predecessor_evidence.py"
    )
    spec = importlib.util.spec_from_file_location("build0055_predecessor", path)
...
def run_import(
    ...
    if git(repo, "status", "--porcelain").stdout.strip():
        raise RuntimeError("repository must be clean before Build 0056 import")
    ...
    backup = backup_root_path / f"Build_0056_Pre_Import_{timestamp}"
    ...
    created: list[str] = []
    predecessor_rel = (
        "Documentation/Build_Records/0056/PREDECESSOR_CANONICAL_EVIDENCE.json"
    )
    register_report_rel = (
        "Documentation/Build_Records/0056/ASSET_REGISTER_CHANGE_REPORT.json"
    )
    import_report_rel = (
        "Documentation/Build_Records/0056/CANONICAL_IMPORT_REPORT.json"
    )
    ...
    try:
        ...
        updater = repo / "Scripts/update_certiaura_build_0056_controls.py"
...
def rollback_from_backup(
    ...
    if str(metadata.get("build_number")) != BUILD_NUMBER:
        raise RuntimeError("transaction metadata does not belong to Build 0056")
    ...
    if unexpected:
        raise RuntimeError(
            "rollback blocked by changes outside Build 0056 ownership: "
            + "; ".join(unexpected)
        )
